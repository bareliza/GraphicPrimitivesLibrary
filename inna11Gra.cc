    ...
    for(j = -BAPi/2; j < BAPi/2; ++j) {
	  ...
	  // ten if nizej to bad coding
          //if ( rgbKrok1 == 0xff0000 || rgbKrok1 == 0xff7f00 || rgbKrok1 == 0xffff00 || rgbKrok1 == 0xff00 )
                  P.prostokat(RX/2 - BX/2 + ((int)ZOOM*i/*fodi*/)-1, 10 + ((int)(ZOOM*rozc1))-1, ZOOM, /*1*/ZOOM-2, rgbKrok1 );
    }
...
int main(int argc, char** argv) {
  ...
  for(int i=0;i<18;++i) {
      P.pisak(i);
      P.linia(10+25*i,10,90+25*i,90,0xff0000);
  }
// "złe" (przesunięte) zakończenia mają, licząc linię numer jeden dla pisaka i=0,
// linie o numerach: 3 4 7 10 11 13 14 17 18
