...
int main(int argc, char** argv) {
  ...
  for(int i = 14; i <= RX-14; i = i + 2*STEP) {
    for(int j = 14; j <= RY-14; j = j + 2*STEP) {
      ...
      P.Linia2(12, RY >> 1,i,j,i+(j<<8)+((i+i)<<16),1 );
      P.Linia2(RX >> 1, RY - 12,i,j,i+(j<<8)+((i+i)<<16),1 );
      P.Linia2(RX - 12, RY >> 1,i,j,i+(j<<8)+((i+i)<<16),1 );

      P.Linia2(i+STEP,j+STEP,RX >> 1, 12,i+(j<<8)+((i+i)<<16),1 );
      P.Linia2(i+STEP,j+STEP,12, RY >> 1,i+(j<<8)+((i+i)<<16),1 );
      P.Linia2(i+STEP,j+STEP,RX >> 1, RY - 12,i+(j<<8)+((i+i)<<16),1 );
      P.Linia2(i+STEP,j+STEP,RX - 12, RY >> 1,i+(j<<8)+((i+i)<<16),1 );
    }
