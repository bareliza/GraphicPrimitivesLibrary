...
void rysuj1(unsigned int *bm, int bx, int by, int xe, int ye) {
  double i, j;
  double fodi;
  for(i = 0; i < X1/*bx*/; ++i)
    for(j = 0; j < by; ++j) {
      ...
      P.prostokat(xe + 4*i, ye + 4*j, 3, 3, bm[((int)j) * by + ((int)fodi)]);
    } 
}
...
void perspektywa(unsigned int *bm, int bx, int by, double xe, double ye) {
  ...
  for(i = 0; i < X1; ++i) {
    ...
    for(j = -BAP/2; j < BAP/2; ++j) {
          P.rysujPunkt(xe + i, ye + (int)j, bm[((int)(rozciagnij((int)j,BAP)))*by         + ((int)fodi)]);
          rgbKrok1 = rgbKrok( ((int)j) % 3 );
	  // ten if nizej to bad coding
          if ( rgbKrok1 == 0xff0000 || rgbKrok1 == 0xff7f00 )
                  P.prostokat(RX/2 - BX/2 + ((int)4.0*i/*fodi*/)-1, 10 + ((int)(4.0*rozciagnij((int)j,BAP)))-1, 3, 3, rgbKrok( ((int)(j / 3) ) % 3 ) );
    }
...
//int main(int argc, char** argv) {
  ...
  rysuj1(bmapa, BX, BY, RX/2 - BX/2, 10); // RY/2 - BY/2);
